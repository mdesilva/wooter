...
use Wooter\Wooter\Transformers\Team\TeamStatsPercentagesTransformer;
use Tymon\JWTAuth\Facades\JWTAuth;

class LeagueOrganizationTeamStatsPercentagesController extends Controller
{
    use Responder, ApiDocErrorBlocs;
    
    private $teamStatsPercentagesTransformer;
    
    public function __construct(TeamStatsPercentagesTransformer $teamStatsPercentagesTransformer)
    {
        $this->teamStatsPercentagesTransformer = $teamStatsPercentagesTransformer;
    ...
    /**
     * @api               {get} api/leagues/:leagueId/team-stats-percentages Index
     * @apiVersion        1.0.0
     * @apiName           Team Stats Percentages
     * @apiGroup          Team Stats Percentages
     * @apiPermission     organization, organization staff, JWTAuth
     * @apiDescription    Returns that stats percentages of teams in a league
     *
     ...
     *
     * @apiSuccess        Object TeamStatsPercentages
     *
    ...
    public function index(Request $request, $leagueId)
    {
        ...
        try {
            ...
            $this->teamStatsPercentagesTransformer->all = $all['stats'];
            $this->teamStatsPercentagesTransformer->sport = $sport;
            $this->teamStatsPercentagesTransformer->collection = true;
            $this->teamStatsPercentagesTransformer->bulk = $bulk;
            
            return $this->respond([
                'data' => $this->teamStatsPercentagesTransformer->transform($result['stats']),
                'pages' => $result['pages']
            ]);
        } catch (Exception $ex) {
        }
