...
class PlayerBasketballStatsRepository
{
    ...
    public function getByPlayerAndLeagueId($player_id, $league_id) {
        return PlayerBasketballStat::whereHas('player', function($query) use($player_id){
    ...
    public function getByGameId($game_id) {
        return PlayerBasketballStat::where('game_id', '=', $game_id)->get();
    }
    
    public function deleteByGameId($game_id) {
        return PlayerBasketballStat::where('game_id', '=', $game_id)
                                   ->delete();
    }
    ...
    public function search($params) {
        ...
        $query->leftjoin('games', 'games.id', '=', 'player_basketball_stats.game_id');
        
        if (isset($params['game_id']) && $params['game_id'] !== '' && $params['game_id'] !== false) {
            $query->where('player_basketball_stats.game_id', '=', $params['game_id']);
        }
        
        $query = $this->filterByPlayerId($query, $params['player_id']);
        
        $statIds = $query->distinct()->lists('player_basketball_stats.id');
        $limit = ($params['limit'] == 'all') ? count($statIds) : $params['limit'];
        $stats = PlayerBasketballStat::whereIn('id', $statIds)
    ...
    public function searchForAverages($params)
    {
        ...
        $statIds = $query->distinct()->lists('player_basketball_stats.id');
        $stats = PlayerBasketballStat::whereIn('id', $statIds)->get();
    ...
    private function getBlankStats($players) {
        ...
        foreach ($players as $player) {
            if ($player->stats->count() === 0) {
                $stats = new PlayerBasketballStat();
                ...
                $stats->{'3_points_shots_made'} = 0;
                $stats->{'3_points_shots_attempted'} = 0;
    ...
    private function filterByCompetitionId($query, $competitionTable, $competitionId) {
     ...
     private function filterByPlayerId($query, $playerId) {
        ...
        if (isset($playerId) && $playerId !== '' && $playerId !== false) {
            $query->where('player_basketball_stats.player_id', '=', $playerId);
        }
     ...
     private function filterByTeamId($query, $teamId) {
        if (isset($teamId) && $teamId !== '' && $teamId !== false) {
            $query->where('player_basketball_stats.team_id', '=', $teamId);
        }
    ...
    private function filterByActive($query, $active) {
        ...
        if (isset($active) && $active !== '' && $active !== false) {
            $query->where('player_basketball_stats.active', '=', $active);
        }
