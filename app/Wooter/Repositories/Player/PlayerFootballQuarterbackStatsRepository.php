...
class PlayerFootballQuarterbackStatsRepository
{

    public function create(PlayerFootballQuarterbackStat $stat)
    {
        return $stat->save();
    }

    public function update(PlayerFootballQuarterbackStat $stat)
    {
        return $stat->save();
    }

    public function getById($stat_id) {
        return PlayerFootballQuarterbackStat::whereId($stat_id)->first();
    }
    
    public function getByLeagueId($league_id) {
        return PlayerFootballQuarterbackStat::whereHas('league', function($query) use($league_id){
    ...
    public function getByPlayerAndLeagueId($player_id, $league_id) {
        return PlayerFootballQuarterbackStat::whereHas('player', function($query) use($player_id){
    ...
    public function getByGameId($game_id) {
        return PlayerFootballQuarterbackStat::where('game_id', '=', $game_id)->get();
    }
    
    public function deleteByGameId($game_id) {
        return PlayerFootballQuarterbackStat::where('game_id', '=', $game_id)
                                   ->delete();
    }
    ...
    public function search($params) {
        ...
        $query->leftjoin('games', 'games.id', '=', 'player_football_quarterback_stats.game_id');
        
        if (isset($params['game_id']) && $params['game_id'] !== '' && $params['game_id'] !== false) {
            $query->where('player_football_quarterback_stats.game_id', '=', $params['game_id']);
        }
        
        $query = $this->filterByPlayerId($query, $params['player_id']);
        
        $statIds = $query->distinct()->lists('player_football_quarterback_stats.id');
        $limit = ($params['limit'] == 'all') ? count($statIds) : $params['limit'];
        $stats = PlayerFootballQuarterbackStat::whereIn('id', $statIds)
    ...
    public function searchForAverages($params)
    {
        ...
        $statIds = $query->distinct()->lists('player_football_quarterback_stats.id');
        $stats = PlayerFootballQuarterbackStat::whereIn('id', $statIds)->get();
    ...
    private function filterByCompetitionId($query, $competitionTable, $competitionId) {
     ...
     private function filterByPlayerId($query, $playerId) {
        ...
        if (isset($playerId) && $playerId !== '' && $playerId !== false) {
            $query->where('player_football_quarterback_stats.player_id', '=', $playerId);
        }
     ...
     private function filterByTeamId($query, $teamId) {
        if (isset($teamId) && $teamId !== '' && $teamId !== false) {
            $query->where('player_football_quarterback_stats.team_id', '=', $teamId);
        }
    ...
    private function filterByActive($query, $active) {
        ...
        if (isset($active) && $active !== '' && $active !== false) {
            $query->where('player_football_quarterback_stats.active', '=', $active);
        }
