...
class TeamBasketballStatsRepository
{

    public function create(TeamBasketballStat $stats)
    {
        return $stats->save();
    }

    public function update(TeamBasketballStat $stats)
    {
        return $stats->save();
    }

    public function getById($id) {
        return TeamBasketballStat::whereId($id)->first();
    }
    
    public function getByGameId($gameId) {
        return TeamBasketballStat::whereGameId($gameId)->get();
    }
    
    public function getByTeamAndGameId($teamId, $gameId) {
        return TeamBasketballStat::whereTeamId($teamId)->whereGameId($gameId)->first();
    }
    
    public function search($params) {
        $returnAllTeams = true;
        $query = DB::table('team_basketball_stats');
        
        $query->leftjoin('games', 'games.id', '=', 'team_basketball_stats.game_id');
        ...
        if (isset($params['teamId']) && $params['teamId'] !== '' && $params['teamId'] !== false) {
            $query->where('team_basketball_stats.team_id', '=', $params['teamId']);
        ...
        $ids = $query->lists('team_basketball_stats.id');
        ...
        $stats = TeamBasketballStat::whereIn('id', $ids)->get();
