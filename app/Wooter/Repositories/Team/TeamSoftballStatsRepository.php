...
class TeamSoftballStatsRepository
{

    public function create(TeamSoftballStat $stats)
    {
        return $stats->save();
    }

    public function update(TeamSoftballStat $stats)
    {
        return $stats->save();
    }

    public function getById($id) {
        return TeamSoftballStat::whereId($id)->first();
    }
    
    public function getByGameId($gameId) {
        return TeamSoftballStat::whereGameId($gameId)->get();
    }
    
    public function getByTeamAndGameId($teamId, $gameId) {
        return TeamSoftballStat::whereTeamId($teamId)->whereGameId($gameId)->first();
    }
    
    public function search($params) {
        $returnAllTeams = true;
        $query = DB::table('team_softball_stats');
        
        $query->leftjoin('games', 'games.id', '=', 'team_softball_stats.game_id');
        ...
        if (isset($params['teamId']) && $params['teamId'] !== '' && $params['teamId'] !== false) {
            $query->where('team_softball_stats.team_id', '=', $params['teamId']);
        ...
        $ids = $query->lists('team_softball_stats.id');
        ...
        })->orderBy($params['orderBy'], $params['orderDirection'])->get(); 
        
        $totalTeamIds = $teams->lists('id');
        $teams = $teams->slice($params['offset'], $limit);
        $teamIds = $teams->lists('id');
        $stats = TeamSoftballStat::whereIn('id', $ids)->get();
    ...
    private function getBlankStats($teams, $teamIds) {
        ...
        foreach ($teams as $team) {
            if (!in_array($team->id, $teamIdsArray)) {
                $stats = new TeamSoftballStat();
                $stats->team_id = $team->id;
                $stats->game_id = 0;
                $stats->first_inning_score = 0;
                $stats->second_inning_score = 0;
                $stats->third_inning_score = 0;
                $stats->fourth_inning_score = 0;
                $stats->fifth_inning_score = 0;
                $stats->sixth_inning_score = 0;
                $stats->seventh_inning_score = 0;
                $stats->eight_inning_score = 0;
                $stats->ninth_inning_score = 0;
                $stats->final_score = 0;
                $stats->win = 0;
                $stats->loss = 0;
                $stats->draw = 0;
