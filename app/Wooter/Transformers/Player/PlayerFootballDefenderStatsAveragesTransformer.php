...
class PlayerFootballDefenderStatsAveragesTransformer extends Transformer
{
    /*
     * @var
     */
    ...
    public function transform($stats)
    {
        if ($this->stat_name) {
            ...
            $response = [
                ...
                'R_rank' => $this->where('R', '>', $stats->avg('pR')),
    ...
    private function where($field, $operator, $value) {
        ...
        $players = $query->whereIn('player_football_defender_stats.player_id', $ids)
                ->groupBy('player_football_defender_stats.player_id')
                ->select('player_football_defender_stats.player_id', DB::raw('AVG(player_football_defender_stats.' . $field . ') as average'))
                ->lists('player_football_defender_stats.average');
