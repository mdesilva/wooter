...
__Wooter.controller('Modals/Dashboard/Photos', ['$scope', 'Page', '$stateParams', '$mdDialog', 'Notify', 'API', function ($scope, Page, $stateParams, $mdDialog, Notify, API) {
    ...
    $scope.publishPhoto = function()
    {
        ...
        if(count($scope.photos) > 0 || $scope.edit) {
            ...
            $leaguePhotosAPI.save({leagueId: leagueId}, data, function (resp) {
    ...
    $scope.remove = function()
    {
        ...
        data = {};
        data.photos = $scope.uploadVideo;
      if(count($scope.uploadVideo) > 0 || $scope.delete )
      {
          $scope.uploadVideo.map(function ( photo ) {
   ...
   function createFilterFor(query)
   {
    ...
    if( count($scope.photoToEdit) > 0 || count($scope.photoToDelete) > 0 )
    {
        if( count($scope.photoToEdit) > 0)
        {
            ...
            var teams = $scope.photoToDelete.tagTeams;
            var players = $scope.photoToDelete.tagPlayers;
    ...
    $scope.createPhotoAlbums = function()
    {
        ...
        $leaguePhotoAlbumsAPI.save({leagueId:leagueId}, data, function(response){

            $leaguePhotoAlbumsAPI.get({leagueId:leagueId}, function(response){
